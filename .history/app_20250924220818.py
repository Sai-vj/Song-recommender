...
# create client creds manager once (for fallback dev searches)
...
def ensure_token():
    ...
    if sp_oauth.is_token_expired(token_info):
        try:
            ...
            app.logger.info("token_info: %s", token_info)
...
def recommend():
    ...
    # create Spotify client depending on auth availability
    if token_info:
        sp = Spotify(auth=token_info["access_token"])
    else:
        # fallback to client credentials (public search only) for dev
        app.logger.info("No user token — using Client Credentials fallback for search")
    ...
    if request.method == "POST":
        ...
        for p in items:
            ...
            # only save to DB when using user flow or when URL exists
